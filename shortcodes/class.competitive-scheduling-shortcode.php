...
if( ! class_exists('Competitive_Scheduling_Shortcode')){
    class Competitive_Scheduling_Shortcode{
        ...
        private function shortcode_page( $page ){
            ...
            if( isset( $_REQUEST['schedule'] ) ){
                ...
                if( ! $return['completed'] ){
                    ...
                    // Redirects the page to previous appointments.
                    wp_redirect( get_permalink(), 301, array( 'window' => 'previous-appointments' ) );
                }
        ...
        private function confirmation(){
            ...
            if( ! $schedules ){
                ...
                if(
                    ...
                    if(
                        ...
                        $msgAgendamentoExpirado = modelo_var_troca_tudo($msgAgendamentoExpirado,"#date_confirmation_1#",$date_confirmation_1);
                        $msgAgendamentoExpirado = modelo_var_troca_tudo($msgAgendamentoExpirado,"#date_confirmation_2#",$date_confirmation_2);
                ...
                if(isset($_REQUEST['efetuar_confirmacao'])){
                    ...
                    $retorno = api_servidor_interface(Array(
                        ...
                        'dados' => Array(
                            ...
                            'id_hosts_usuarios' => $_MANAGER['usuario-id'],
                        ),
                    ));
                    ...
                    if(!$retorno['completed']){
                        ...
                        if(isset($dados['schedules_dates'])){
                            $id_hosts_schedules_dates = $dados['schedules_dates']['id_hosts_schedules_dates'];
                            $total = $dados['schedules_dates']['total'];
                            
                            $schedules_dates = banco_select(Array(
                                'unico' => true,
                                'tabela' => 'schedules_dates',
                                'campos' => Array(
                                    'id_hosts_schedules_dates',
                                ),
                                'extra' => 
                                    "WHERE id_hosts_schedules_dates='".$id_hosts_schedules_dates."'"
                            ));
                            
                            if($schedules_dates){
                                banco_update_campo('total',$total);
                                
                                banco_update_executar('schedules_dates',"WHERE id_hosts_schedules_dates='".$id_hosts_schedules_dates."'");
                            } else {
                                $date = $dados['schedules_dates']['data'];
                                $status = $dados['schedules_dates']['status'];
                                
                                banco_insert_name_campo('id_hosts_schedules_dates',$id_hosts_schedules_dates);
                                banco_insert_name_campo('data',$date);
                        ...
                        $id_hosts_usuarios = $dados['schedules']['id_hosts_usuarios'];
                        $id_hosts_schedules = $dados['schedules']['id_hosts_schedules'];
                        ...
                        $schedules = banco_select(Array(
                            ...
                            'extra' => 
                                "WHERE id_hosts_schedules='".$id_hosts_schedules."'"
                        ));
                        
                        if($schedules){
                            // Atualizar agendamento.
                            
                            if(isset($dados['schedules'])){
                                $schedules = $dados['schedules'];
                                
                                foreach($schedules as $key => $valor){
                                    switch($key){
                                        case 'acompanhantes':
                                        case 'versao':
                                ...
                                banco_update_executar('schedules',"WHERE id_hosts_schedules='".$id_hosts_schedules."' AND id_hosts_usuarios='".$id_hosts_usuarios."'");
                            }
                        }
                ...
                $_MANAGER['javascript-vars']['confirmar'] = true;
            }
            ...
            pagina_trocar_variavel_valor('confirmacao-data',($schedules ? formato_dado_para('data',$schedules['data']) : ''));
            pagina_trocar_variavel_valor('confirmacao-agendamento-id',$id_hosts_schedules);
        ...
        private function cancellation(){
            ...
            $id_hosts_schedules = (isset($_REQUEST['schedule_id']) ? sanitize_text_field($_REQUEST['schedule_id']) : '');
            ...
            $schedules = banco_select(Array(
                ...
                'extra' => 
                    "WHERE id_hosts_schedules='".$id_hosts_schedules."'"
                    ." AND id_hosts_usuarios='".$_MANAGER['usuario-id']."'"
            ));
            ...
            if(!$schedules){
                ...
                if(isset($_REQUEST['efetuar_cancelamento'])){
                    ...
                    $retorno = api_servidor_interface(Array(
                        ...
                        'opcao' => 'cancelar',
                        'dados' => Array(
                            'id_hosts_schedules' => $id_hosts_schedules,
                            'id_hosts_usuarios' => $_MANAGER['usuario-id'],
                        ),
                    ));
                ...
                $_MANAGER['javascript-vars']['cancelar'] = true;
            }
            ...
            pagina_trocar_variavel_valor('cancelamento-data',($schedules ? formato_dado_para('data',$schedules['data']) : ''));
            pagina_trocar_variavel_valor('cancelamento-agendamento-id',$id_hosts_schedules);
        ...
        private function do_schedule( $params = false ){
            if( $params ) foreach( $params as $var => $val ) $$var = $val;
            
            // Get user ID
            $data['userID'] = get_current_user_id();

            // Check if the mandatory fields were sent: userID and scheduleDate.
            if( isset( $data['userID'] ) && isset( $data['scheduleDate'] )){
                // Get the configuration data.
                $options = get_option( 'competitive_scheduling_options' );
                $msg_options = get_option( 'competitive_scheduling_msg_options' );
                ...
                $companionsNames = $data['companionsNames'];
