jQuery( document ).ready( function () {
	...
	function priority_coupons() {
		...
		jQuery( '.printCoupons' ).on( 'mouseup tap', function ( e ) {
			...
			window.open(
				manager.root + 'pagina-de-impressao/',
				'Imprimir',
				'menubar=0,location=0,height=700,width=1024'
			);
		} );
	}
	...
	function schedules_update( p = {} ) {
		...
		// Modify as submitted.
		if ( 'date' in p ) {
			manager.schedules.date = p.date;
		}
		if ( 'status' in p ) {
			manager.schedules.status = p.status;
		}
		...
		if ( 'date' in manager.schedules ) {
			jQuery( '.resultados' ).show();
		}

		// Only update if 'data' and 'status' are defined.
		if ( 'date' in manager.schedules && 'status' in manager.schedules ) {
			...
			jQuery
				...
				.done( function ( response ) {
					...
					if ( 'nonce' in response ) {
						jQuery( 'input[name="schedules-nonce"]' ).val(
							response.nonce
						);
					}
			...
			if ( lastButton !== obj ) {
				jQuery( this )
					...
					.each( function () {
						...
						jQuery( this )
							.find( 'i' )
							.removeClass( 'square outline icon' );

						if ( this !== obj ) {
							jQuery( this )
								.find( 'i' )
								.addClass( 'square outline icon' );
						}
					} );

				jQuery( this )
					.find( 'i' )
					.addClass( 'check square outline icon' );
	...
	function start() {
		...
		if (
			jQuery( '#_gestor-interface-edit-dados' ).length > 0 ||
			jQuery( '#_gestor-interface-insert-dados' ).length > 0
		) {
			priority_coupons();
		}
	}
