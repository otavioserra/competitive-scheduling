jQuery( document ).ready( function () {
	function confirmPublic() {
		...
		// Confirmation button.
		jQuery( '.confirmPublicSchedulleBtn' ).on(
			'mouseup tap',
			function ( e ) {
		...
		// Cancel button.
		jQuery( '.cancelPublicSchedulingBtn' ).on(
			'mouseup tap',
			function ( e ) {
				if ( e.which != 1 && e.which != 0 && e.which != undefined )
					return false;
	...
	function start() {
		...
		if ( 'confirm' in cs_manager ) {
			confirmPublic();
		}
		if ( 'cancel' in cs_manager ) {
			cancelPublic();
		}
		if ( 'expiredOrNotFound' in cs_manager ) {
			expiredOrNotFound();
		}
		if ( 'errorInfo' in cs_manager ) {
			errorInfo();
		}
		if ( 'successInfo' in cs_manager ) {
			successInfo();
		}
		if ( 'calendarShortcode' in cs_manager ) {
			calendarShortcode();
		}
	}
			...
			case 'open':
				if ( ! ( 'loading' in cs_manager ) ) {
					jQuery( '.page.dimmer' ).dimmer( {
						closable: false,
						onVisible: function () {
							jQuery( '.page.dimmer' ).removeClass(
								'transition'
							);
						},
					} );
